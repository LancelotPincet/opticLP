...
Modules go in `src/opticlp/modules/`.
Routines go in `src/opticlp/routines/`.
...
- Public objects must be exposed in `opticlp/__init__.py`
...
```python
# RIGHT
from opticlp import MyObject

# WRONG — never import from internal paths
from opticlp.modules.myobject_LP.myobject import MyObject
```

Every public object must be registered in `opticlp/__init__.py`:

```python
sources = {
    "<name>": "opticlp.modules.<name>_LP.<name>",
}
```
